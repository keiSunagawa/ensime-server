...
trait StandardFormats {
  ...
  implicit def eitherFormat[A: JF, B: JF] = new JF[Either[A, B]] {
    ...
    def read(value: JsValue) =
      (value.convertTo(safeReader[A]), value.convertTo(safeReader[B])) match {
  ...
  implicit def tuple3Format[A: JF, B: JF, C: JF] =
    new RootJsonFormat[(A, B, C)] {
      ...
      def read(value: JsValue) = value match {
        ...
        case x =>
          deserializationError("Expected Tuple3 as JsArray, but got " + x)
      }
    }

  implicit def tuple4Format[A: JF, B: JF, C: JF, D: JF] =
    new RootJsonFormat[(A, B, C, D)] {
      def write(t: (A, B, C, D)) =
        JsArray(t._1.toJson, t._2.toJson, t._3.toJson, t._4.toJson)
      def read(value: JsValue) = value match {
        case JsArray(Seq(a, b, c, d)) =>
          (a.convertTo[A], b.convertTo[B], c.convertTo[C], d.convertTo[D])
        case x =>
          deserializationError("Expected Tuple4 as JsArray, but got " + x)
      }
    }
  ...
  implicit def tuple5Format[A: JF, B: JF, C: JF, D: JF, E: JF] =
    new RootJsonFormat[(A, B, C, D, E)] {
      def write(t: (A, B, C, D, E)) =
        JsArray(t._1.toJson, t._2.toJson, t._3.toJson, t._4.toJson, t._5.toJson)
      def read(value: JsValue) = value match {
        case JsArray(Seq(a, b, c, d, e)) =>
          (a.convertTo[A],
           ...
           e.convertTo[E])
        case x =>
          deserializationError("Expected Tuple5 as JsArray, but got " + x)
      }
    }
      ...
      def read(value: JsValue) = value match {
        case JsArray(Seq(a, b, c, d, e, f)) =>
          (a.convertTo[A],
           ...
           f.convertTo[F])
        case x =>
          deserializationError("Expected Tuple6 as JsArray, but got " + x)
      }
    }
      ...
      def read(value: JsValue) = value match {
        case JsArray(Seq(a, b, c, d, e, f, g)) =>
          (a.convertTo[A],
           ...
           g.convertTo[G])
        case x =>
          deserializationError("Expected Tuple7 as JsArray, but got " + x)
      }
    }
