...
class DocsHandler(docs: DocJarReading)
    ...
    // Allow only GET methods.
      ...
      req.uri() match {
        ...
        case DocEntry(filename, entry) =>
          ...
          docs
            .docJarContent(filename, entry)
            .fold(sendHttpResponse(ctx, req, Left(NOT_FOUND))) { jarContent =>
  ...
  private def sendHttpResponse(
    ...
    val res = statusOrContent match {
      ...
      case Right((mimeTypeOpt, content)) =>
        ...
        mimeTypeOpt.foreach { mimeType =>
          res.headers().set(CONTENT_TYPE, s"$mimeType; charset=UTF-8")
        }
        res
    }
