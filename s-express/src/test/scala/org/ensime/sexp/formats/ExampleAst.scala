...
object ExampleAst {
  ...
  final case class AndCondition(left: TokenTree, right: TokenTree, text: String)
      extends TokenTree
  final case class OrCondition(left: TokenTree, right: TokenTree, text: String)
      extends TokenTree
  ...
  final case class FieldTerm(text: String, field: DatabaseField, value: String)
      extends Term
  ...
  final case class PreferToken(tree: TokenTree,
                               before: Option[Prefer],
                               after: Option[Prefer])
      extends TokenTree {
    val text = before.getOrElse("") + tree.text + after.getOrElse("")
  }
  final case class InTerm(field: DatabaseField,
                          value: List[String],
                          text: String = "")
      extends CompressedToken

  final case class QualifierToken(text: String, field: DatabaseField)
