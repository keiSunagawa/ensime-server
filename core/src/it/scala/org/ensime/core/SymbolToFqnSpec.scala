...
class SymbolToFqnSpec
  ...
  "SymbolToFqn" should "calculate the FQNs from Symbols.Symbol" in withPresCompiler {
    (config, cc) =>
      runForPositionInCompiledSource(
        ...
        cc.askSymbolFqn(p).getOrElse { fail(label) } shouldBe {
          label match {
            ...
            case "nested_class" =>
              clazz(Seq("com", "example"), "Outer$Inner$Nested")
          }
        }
      }
  }

  it should "calculate the FQNs from the typeSymbol of Types.Type" in withPresCompiler {
    (config, cc) =>
      runForPositionInCompiledSource(
        config,
        cc,
  ...
  it should "calculate the FQN for imports" in withPresCompiler {
    (config, cc) =>
      import ReallyRichPresentationCompilerFixture._
      runForPositionInCompiledSource(
        config,
