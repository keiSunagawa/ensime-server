...
class SearchServiceSpec
  ...
  it should "not prioritise noisy inner classes" in withSearchService {
    implicit service =>
      def nonTrailingDollarSigns(fqn: String): Int =
        fqn.count(_ == '$') - (if (fqn.endsWith("$")) 1 else 0)
      def isSorted(hits: Seq[String]): Boolean =
        hits.sliding(2).forall {
          case List(x, y) =>
            nonTrailingDollarSigns(x) <= nonTrailingDollarSigns(y)
        }
      ...
      val sorted = new BeMatcher[Seq[String]] {
        override def apply(left: Seq[String]): MatchResult =
          MatchResult(
            isSorted(left),
            s"Elements of $left were not sorted by the amount of non trailing $$'s. ${left
              .map(nonTrailingDollarSigns)}",
  ...
  it should "find usages of a regular class" in withSearchService {
    implicit service =>
  ...
  it should "find usages of a field/method" in withSearchService {
    implicit service =>
      val fieldUsages =
        findUsages("org.reverselookups.ReverseLookups.intField()I")
      ...
      val methodUsages =
        findUsages("org.reverselookups.ReverseLookups.catches()I")
  ...
  it should "detect uses of outer classes in inner class calls" in withSearchService {
    implicit service =>
      val barUsages = findUsages("org.example.Bar$")
      barUsages.map(mn => unifyMethodName(mn.toSearchResult)) should contain(
        "Method org.example.Qux#<init>(): org.example.Qux"
      )
  }

  "scala names" should "be correctly resolved for overloaded methods" in withSearchService {
    implicit service =>
      val hits = service
      ...
      hits.map(hit => hit.fqn -> unifyMethodName(hit.toSearchResult)) should contain theSameElementsAs List(
        ("org.reverselookups.Overloads.foo()V",
         "Method org.reverselookups.Overloads#foo(): scala.Unit"),
        ("org.reverselookups.Overloads.foo(I)V",
         "Method org.reverselookups.Overloads#foo(i: scala.Int): scala.Unit"),
        ("org.reverselookups.Overloads.foo(Ljava/lang/String;I)V",
         "Method org.reverselookups.Overloads#foo(s: scala.Predef.String, i: scala.Int): scala.Unit"),
        ("org.reverselookups.Overloads.foo(Lorg/reverselookups/MyException;)V",
         "Method org.reverselookups.Overloads#foo[T <: org.reverselookups.MyException](t: T): scala.Unit"),
        ("org.reverselookups.Overloads.foo(Lorg/reverselookups/MyAnnotation;)V",
         "Method org.reverselookups.Overloads#foo(ann: org.reverselookups.MyAnnotation): scala.Unit")
      )
  }
