...
class PositionLocator(val global: RichPresentationCompiler)
      ...
      case _ =>
        if (t.pos properlyIncludes pos) {
          if (isEligible(t)) {
            ...
            last =
              if (namePosition properlyIncludes pos) namePosition else t.pos
          }
          super.traverse(t)
        } else
          t match {
  ...
  def enclosingTreePosition(p: Position): Position =
    new ProperlyIncludingLocator(p).locateIn(parseTree(p.source))
}
