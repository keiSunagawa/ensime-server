...
class JavaCompiler(
  ...
  lazy val fileManager: JavaFileManager =
    compiler.getStandardFileManager(listener, null, charset)
  ...
  protected def pathToPoint(file: SourceFileInfo,
                            offset: Int): Option[(Compilation, TreePath)] = {
    val infos = typecheckForUnits(List(file))
    infos.headOption.flatMap { c =>
  ...
  protected def scopeForPoint(file: SourceFileInfo,
                              offset: Int): Option[(Compilation, Scope)] = {
    val infos = typecheckForUnits(List(file))
    infos.headOption.flatMap { c =>
